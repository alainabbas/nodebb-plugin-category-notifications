...
categoryNotifications.isSubscribed = async function (uid, cid) {
	...
	settings.subscription = settings.subscription || 'never';
	if (settings.subscription === 'never'){
		return await db.isSortedSetMember(`cid:${cid}:subscribed:uids`, uid);
	}else if (settings.subscription === 'always'){
		return ! await db.isSortedSetMember(`cid:${cid}:unsubscribed:uids`, uid);
	}
...
categoryNotifications.subscribe = async function (uid, cid) {
	const settings = await meta.settings.get('category-notifications');
	settings.subscription = settings.subscription || 'always';
	if (settings.subscription === 'never') {
		await db.sortedSetAdd(`cid:${cid}:subscribed:uids`, Date.now(), uid);
	}else if (settings.subscription === 'always'){
		await db.sortedSetRemove(`cid:${cid}:unsubscribed:uids`, uid);
	}
};
...
categoryNotifications.unsubscribe = async function (uid, cid) {
	...
	if (settings.subscription === 'never') {
		...
		await db.sortedSetAdd(`cid:${cid}:unsubscribed:uids`,  Date.now(), uid);
	}
};
...
categoryNotifications.onUserDelete = async function (data) {
	...
	await db.sortedSetsRemove(
		cids.map(cid => `cid:${cid}:unsubscribed:uids`),
		data.uid
	);
};
...
async function getSubscribers(cid, exceptUid) {
	...
	settings.subscription = settings.subscription || 'never';
	if (settings.subscription === 'never') {
		let uids = await db.getSortedSetRange(`cid:${cid}:subscribed:uids`, 0, -1);
		return uids.filter(uid => parseInt(uid, 10) !== parseInt(exceptUid, 10));
	}else if (settings.subscription === 'always'){
		const usersList=await db.getSortedSetRangeWithScores(`username:uid`, 0, -1);
		const noUids = await db.getSortedSetRange(`cid:${cid}:unsubscribed:uids`, 0, -1);
		let uids =[];
		let category=null;

		for( const user of usersList) {

			const userPrivileges= await privileges.categories.get(cid,user.score)
			console.log("Log: " + userPrivileges.read + " " + cid + " " +user.score)
			if ( noUids.indexOf(user.score.toString()) === -1 && exceptUid !== user.score  && userPrivileges.read){
				uids.push(user.score)
			}

		}
		console.log('liste desuids');
		console.dir(uids);
        return uids
	}
}
